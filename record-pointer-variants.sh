...
OUT="${HOME}/test-pointer-tail-variants.mp4"
WORK="$(mktemp -d)"
...
WEBM="$(find "$ROOT/test-results" -name 'pointer-variants.webm' | head -1)"
[[ -n "$WEBM" ]] || { echo "missing pointer-variants.webm" >&2; exit 1; }

ffmpeg -y -i "$WEBM" -vf "fps=${EVIDENCE_OUTPUT_FPS}" \
  -c:v libx264 -preset slow -crf 8 -pix_fmt yuv420p -movflags +faststart -an "$WORK/out.mp4"
mv "$WORK/out.mp4" "$OUT"
rm -rf "$WORK"
printf '\nDone: %s (tail XS → XL, #3 = wing v2)\n' "$OUT"
